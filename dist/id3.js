				...
				handle.read(tagSize, headerSize, function(err, buffer) {
					...
					while(position < buffer.byteLength) {
						...
						if(tags.v2.version[0] < 3) {
							var frameSize = ((dv.getUint8(position + 5) & 0xFF) << 8) | ((dv.getUint8(position + 4) & 0xFF) << 16) | ((dv.getUint8(position + 3) & 0xFF) << 24);
							slice = buffer.slice(position, position + 6 + frameSize);
						} else {
